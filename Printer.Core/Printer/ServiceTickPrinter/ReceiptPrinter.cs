...
namespace Printer.Core.Printer.ServiceTickPrinter
{
    ...
    public class ReceiptPrinter:PrinterBase
    {
            ...
            try
            {
                ...
                if (result)
                {
                    ...
                    for (int i = 0, p = 1, l = serviceReceipt.Orders.Count; i < l; i += pageMax, p++)
                    {
                        ...
                        PrintTable(p);
                        SendData2USB("\r\n");
                        SendData2USB(Core.Printer.PrinterCmdUtils.lineSpace(60));
        ...
        private void PrintHead(int page)
        {
            ...
            SendData2USB($"门店：{StringHelper.GetAdjustedString(serviceReceipt.Store, serviceReceipt.Date.ToString("yyyy-MM-dd"))}");
            PrintTitleSp();
        ...
        private void PrintTableBody(int page)
        {
            ...
            for (int i = 0,l = orders.Count(); i < l; i++)
            {
                ...
                SendData2USB($@"┃{StringHelper.GetIntFix(index)}┃{item.NoBytes}┃{item.RecieverBytes}┃{item.RecieverPhoneBytes}┃");
                NextLine();
                SendData2USB("┃  ┣━━━━━━━╋━━━━━╋━━━━━━┫");
                NextLine();
